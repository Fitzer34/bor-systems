...
The enclosures in `hardware/enclosures/v9` (design v9.10) are sized for these parts. Nothing in that design is 21700. The sizes come from the makers' own drawings, 3D models and listings, and `hardware/enclosures/v9/PRINT.md` lists which document gave which number under "Where the sizes come from". Nothing on this list has to be measured or photographed.
...
| BeiLaMoo BH18650-PC2 holder, sold as "1x 18650 Battery holder for PCB" | 4 | TinyTronics, order 660112 | Delivered. 77.7 x 20.9 x 21.3 mm, one solder pin at each end. It is a copy of the MPD BH-18650-PC, and MPD's drawing gives the pins: 3.30 mm long, 2.40 mm in from each end. One per hanger. Its pins end up behind it, so the battery wires are soldered on BEFORE it is pressed into the box. Thread the two wires down through the bulkhead first, see Order #3 |
...
The button goes UNDER the base of the hanger, through a 16.4 mm hole in the bottom wall of the hanger body, on the right-hand side behind the right latch arm. It points down, above the hook, where the public cannot see it from the front. A cleaner reaches under the box from the right-hand side and presses it upward. There is about 42.8 mm between the button's face and the hook for a finger.

What the part is, from Gebildet's own listing drawing: an M16 x 1 thread for a 16 mm hole, a head 18.8 mm across, and 46 mm overall with the blue plug-in wire socket pushed on. Five wires leave the end of the socket. The enclosure allows 44.5 mm behind the head, which is the 46 mm less the 1.6 mm head, and 3.2 mm under the base for the head on its sealing ring. It comes with its own nut.

To make room for that length under the battery, the hanger box grew downward by 44 mm: the bay under the battery is 44 mm and the hanger body is 174 mm tall in v9.10 (it was 130 before the button was added). The gateway keeps its size.
...
The five wires. Only TWO of them are used: the two switch wires. Find them on the bench with a meter set to continuity (the beep setting), by trying the wires in pairs until one pair beeps only while the button is held in. Gebildet's listing picture shows green as the common wire and yellow as the normally open one, with white as the normally closed one and red and black for the LED. That picture is a stock one shared between listings, so go by the meter. Those two go up to the board, one to GND and one to GPIO3, either way round. The other three are folded back, their bare ends covered with heat shrink or tape, and tucked down in the bay under the battery, connected to nothing. Deal with all five BEFORE the battery holder is pressed in: they leave the top of the socket pointing up at the place where the holder goes. Check that the two switch wires reach the join below the board, and lengthen them with hook-up wire if they do not.

The button's LED is a 12 to 24 V type per Gebildet's listing, so it will not light from the board's 3.3 V. Its wires stay unconnected. Only the two switch wires are used.
...
Check each of these is in hand before starting, because none of them is on an order. The one marked Optional can be left out:
...
- White 2.85 mm PLA. Owen's colour choice is a black box with the hook bar, the logo and the button names in white. The white spool is on the printer's nozzle 1 and the black PLA is on nozzle 2. The whole hook bar prints in white, and the lettering is a white inlay merged with the black lid (see Colours in `hardware/enclosures/v9/PRINT.md`). The white spool is on no order in this file.
...
The orders do not cover everything. The hook-up wire, the capacitors, the heat shrink, the cable ties, the gateway's window pane, the wall fixings and the white PLA are listed near the top under "Things the build needs that are not on any order below", along with the optional plug and socket pairs.
