...
/**
 * Security — guard checkpoints + patrols. Define QR-tagged checkpoints at a
 * site; print the QR and stick it up; guards scan it on rounds (no login) and
 * each scan is logged here as the patrol record.
 */
...
export function Checkpoints() {
  ...
  const cps = useQuery({ queryKey: ["checkpoints"], queryFn: () => api<{ checkpoints: Checkpoint[] }>("/checkpoints") });
  const scans = useQuery({ queryKey: ["checkpoint-scans"], queryFn: () => api<{ scans: Scan[] }>("/checkpoint-scans"), refetchInterval: 30_000 });
  ...
  return (
    <div className="p-6 max-w-6xl">
      <div className="flex items-center justify-between mb-6 gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-semibold">Guard checkpoints</h1>
          <p className="text-sm text-slate-500 mt-1">Print a checkpoint's QR and place it on site. Guards scan it on patrol — every scan is logged below.</p>
        </div>
        <button onClick={() => setCreating(true)} className="px-3 py-1.5 text-sm bg-blue-600 hover:bg-blue-500 rounded text-white font-medium whitespace-nowrap">+ Add checkpoint</button>
      </div>

      {list.length === 0 ? (
        <div className="rounded-lg border border-slate-200 bg-white p-8 text-center">
          <p className="text-slate-800">No checkpoints yet.</p>
          <p className="text-sm text-slate-500 mt-2">Add points guards should visit (fire exits, plant rooms, perimeter gates…) and print each QR.</p>
        </div>
      ) : (
      ...
      <h2 className="text-xs uppercase tracking-wider text-slate-500 mt-10 mb-2">Recent patrols</h2>
      {scanList.length === 0 ? (
        ...
        <div className="rounded-lg border border-slate-200 bg-white divide-y divide-slate-100">
          {scanList.map((s) => (
            <div key={s.id} className="px-4 py-2.5 flex items-center gap-3 text-sm flex-wrap">
              ...
              <span className="text-slate-500">· {s.guardName || "Guard"}</span>
      ...
      {(editing || creating) && (
        <CheckpointDialog
          cp={editing} buildings={buildings}
...
function CheckpointDialog({ cp, buildings, onClose, onSaved }: {
  cp: Checkpoint | null; buildings: Building[]; onClose: () => void; onSaved: () => void;
}) {
  const isEdit = !!cp;
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-white rounded-xl w-full max-w-lg border border-slate-300 shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
        <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between">
          <h2 className="text-lg font-medium text-slate-900">{isEdit ? "Edit checkpoint" : "Add checkpoint"}</h2>
        ...
        <div className="px-6 py-5 space-y-4 max-h-[70vh] overflow-y-auto">
          <Group label="Checkpoint name">
            <input value={name} onChange={(e) => setName(e.target.value)} autoFocus maxLength={160} placeholder="e.g. Rear fire exit" className={inp} />
          </Group>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <Group label="Instructions on scan (optional)">
            <textarea value={instructions} onChange={(e) => setInstructions(e.target.value)} rows={2} maxLength={2000} placeholder="What the guard should check / do here — shown when they scan." className={inp + " resize-none"} />
          </Group>
        ...
        <div className="px-6 py-4 border-t border-slate-200 flex items-center justify-between gap-3">
          ...
          <div className="flex gap-2 ml-auto">
            ...
            <button onClick={() => { setErr(null); save.mutate(); }} disabled={!name.trim() || save.isPending} className="px-4 py-1.5 text-sm bg-blue-600 hover:bg-blue-500 disabled:bg-slate-200 disabled:text-slate-500 rounded text-white font-medium">
              {save.isPending ? "Saving…" : isEdit ? "Save" : "Add checkpoint"}
            </button>
          </div>
        </div>
      </div>
