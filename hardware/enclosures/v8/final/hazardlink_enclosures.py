#!/usr/bin/env python
"""HazardLink v8 enclosures, FINAL (Designer A base, hanger reworked to Owen's bottom-hung concept).
...
P = dict(
    ...
    BULK_Z=(34.0, 46.0),              # assumed: stiffening bulkhead between bay and board (kept, no inserts in it now)
    ...
    BAR_WEB_Y=(22.0, 32.0),  # assumed: the web that carries the bar down from the body, under the back of the bay
    BAR_PLATE_T=4.0,         # assumed: mounting plate under the body (1.5 of it is the key inside the pocket)
    ...
    BAR_KEY_D=1.5, BAR_KEY_CLR=0.25,      # assumed: locating key into the body bottom wall (depth, clearance per side)
    BAR_KEY_Y=(3.0, 32.0),   # assumed: key span along Y under the body (inside the walls)
    BAR_SCREWS=((43.0, 8.0), (57.0, 8.0)),     # assumed: two M3 inserts up into the body bottom wall, behind the lid lip and in front of the holder (x, Y)
    BAR_INSERT_DEPTH=4.7, BAR_BOSS_TOP=5.7,     # assumed: short ruthex M3 x 4 insert; boss top stays under the holder (z<6)
    BAR_WIRE_Y=(24.0, 30.0), # assumed: lead slot through the bottom wall above the web; leads run forward in the gap under the holder
...
def hanger_geom():
    ...
    g["bar_back"] = g["web_y1"]
    g["plate_bot"] = P["BAR_KEY_D"] - P["BAR_PLATE_T"]
    ...
    g["key_y0"], g["key_y1"] = P["BAR_KEY_Y"]
    g["bar_screws"] = P["BAR_SCREWS"]
    return g
...
def build_hanger_body():
    ...
    # hanging bar interface on the bottom wall: two insert bosses (up into the body), a keyed pocket
    # on the outer face that takes the bar's key, and the Hall lead slot in front of the holder
    for (bx, by) in g["bar_screws"]:
        body = body.union(cyl_z(bx, by, P["BOSS_D"], wall - EPS, P["BAR_BOSS_TOP"]))
        body = body.cut(cyl_z(bx, by, P["INSERT_M3_D"], -1.0, P["BAR_INSERT_DEPTH"]))
    kc = P["BAR_KEY_CLR"]
    body = body.cut(box(g["bar_x0"] - kc, g["bar_x1"] + kc, g["key_y0"] - kc, g["key_y1"] + kc, -1.0, P["BAR_KEY_D"]))
    wy0, wy1 = P["BAR_WIRE_Y"]
    body = body.cut(box(W / 2 - P["BAR_WIRE_W"] / 2, W / 2 + P["BAR_WIRE_W"] / 2, wy0, wy1, -1.0, wall + 1.0))
...
def build_hanger_bar():
    """Bolt-on hanging bar: a mounting plate (its top 1.5 mm is a key that drops into the body's bottom-wall pocket,
    two M3 screws up into inserts), a web at the back that carries the bar down BAR_DROP below the body, the bar
    itself out to an upturned lip, a saddle in the bar's top face where the sign's handle settles, and the Hall
    carrier in a slot directly under the saddle. The sign's top sits in the pocket between bar and body. Prints
    lying on a side (X) face so layers run along the profile."""
    g = hanger_geom()
    x0, x1 = g["bar_x0"], g["bar_x1"]
    plate = box(x0, x1, g["key_y0"], g["key_y1"], g["plate_bot"], g["bar_top"] * 0 + P["BAR_KEY_D"])
    ...
    hw = g["slot_hw"]
    part = part.cut(box(g["W"] / 2 - hw, g["W"] / 2 + hw, g["slot_y_front"], g["bar_back"] + 1, g["slot_floor"], g["slot_ceil"]))
    # lead channel: along the bar behind the saddle, then up the web into the body's bottom-wall slot
    cw2 = P["BAR_WIRE_W"] / 2 - 1
    part = part.cut(box(g["W"] / 2 - cw2, g["W"] / 2 + cw2, g["saddle_y"] + sw / 2, g["bar_back"] + 1, g["wire_z0"], g["slot_ceil"]))
    wy0, wy1 = P["BAR_WIRE_Y"]
    part = part.cut(box(g["W"] / 2 - cw2, g["W"] / 2 + cw2, wy0 + 0.5, wy1 - 0.5, g["wire_z0"], P["BAR_KEY_D"] + 1))
    # two counterbored M3 button-head screws from under the plate, up into the body inserts
    for (sx, sy_) in g["bar_screws"]:
        part = part.cut(cyl_z(sx, sy_, P["SCREW_M3_CLR"], g["plate_bot"] - 1, P["BAR_KEY_D"] + 1))
        part = part.cut(cyl_z(sx, sy_, P["CB_M3_D"], g["plate_bot"] - 1, g["plate_bot"] + 1.0))
    return part
...
def design_checks(hg, gg):
    ...
    for (sx, sy_) in hg["bar_screws"]:
        d = math.sqrt((sx - W / 2) ** 2 + (sy_ - sy) ** 2 + (0.0 - hg["sensor_z"]) ** 2)
        out.append((d >= 15.0, "Hall sensor to bar screw at x=%.0f: %.1f mm (>= 15 assumed rule)" % (sx, d)))
    ...
    gap_under = P["HOLDER_Z0"] - P["HOLDER_CLR"] - P["WALL"]
    out.append((gap_under >= 3.0, "gap under the holder for the Hall leads = %.1f mm (>= 3)" % gap_under))
...
def write_readme(out_dir, hg, gg):
    ...
    txt = """HazardLink v8 enclosures, FINAL. Generated by hazardlink_enclosures.py.
...
and box; the bar reaches %.0f mm out from the wall face and ends in a %.0f mm lip; the sign's handle settles in the saddle %.0f mm
...
""" % (hg["wall_y"], P["BAR_REACH"], P["BAR_LIP_H"], -sy, hg["W"] / 2, sy, P["TAG_CLR"], P["TAG_WALL"], gap)
...
def main(out_dir, quick=False, autocad=True):
    ...
    sy = hg["sensor_y"]
    write_part_drawing(os.path.join(out_dir, "hanger_bar_drawing.dxf"), "HazardLink hanger bar v8 (mm)", h_bar, [
        dict(view="side", at=W / 2, off=(0, -120), label="SIDE SECTION at X=50: bar, lip, key, saddle, Hall slot, lead channel",
        ...
        dict(view="top", at=hg["slot_floor"] + 1.0, off=(230, -120), label="TOP SECTION through the Hall slot: carrier pocket, screw holes, lead channel",
             dims=[("h", (hg["bar_screws"][0][0], -hg["bar_screws"][0][1]), (hg["bar_screws"][1][0], -hg["bar_screws"][1][1]), -hg["bar_screws"][0][1] + 8, "screws <>")],
             refs=[h_refs["hall_carrier"][0]]),
    ], notes=["Print lying on a side face (X face on the bed) so layers run along the bar. PETG. Screws: 2 x M3 x 10 button head from underneath (assumed).",
    ...
    # ---- manifest -----------------------------------------------------------------------
    lines = ["HazardLink v8 enclosures, FINAL. Generated by hazardlink_enclosures.py", ""]
    for name, wp in parts.items():
