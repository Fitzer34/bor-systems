...
export function payloadDecodeError(message: string): Error {
  const err = new Error(message);
  err.name = "PayloadDecodeError";
  return err;
}

export function isPayloadDecodeError(err: unknown): err is Error {
  return err instanceof Error && err.name === "PayloadDecodeError";
}

export function decodePayload(bytes: Uint8Array): DecodedPayload {
  if (bytes.length < 4) {
    throw payloadDecodeError(`expected 4 bytes, got ${bytes.length}`);
  }
  const code = bytes[0]!;
  const eventType = NAME_BY_CODE[code];
  if (!eventType) {
    throw payloadDecodeError(`unknown event_type 0x${code.toString(16)}`);
  }
  const batteryPct = bytes[1]!;
  if (batteryPct > 100) {
    throw payloadDecodeError(`battery_pct out of range: ${batteryPct}`);
  }
